...
\begin{enumerate}
\item Saber la definición formal de {\bf número de condición} y de {\bf número de condición absoluto} en un punto $x\in V$ de una función $f:V\rightarrow W$ entre espacios normados $V$ y $W$.
...
\begin{enumerate}
\item Saber qué es la factorización $LU$.
\item Cómo usarías la factorización $A=LU$ para resolver el sistema $Ax=b$? Qué puedes decir del número de condición y del error relativo de ese algoritmo?
...
\begin{theorem} [Sylvester, factorización de Cholesky] Si $A$ es simétrica y $\det(A_p)>0$ para todo $p$ entonces existe una matriz invertible, triangular inferior $C$ con $A=CC^T$.
\end{theorem}

\item Saber que, por Gauss-Jordan, TODA matriz admite una descomposición $LU$ con pivoteo $PA=LU$ para alguna permutación $P$. 
